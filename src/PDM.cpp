...
static inline uint8_t get_nrf_pin_num(uint8_t pin)
{
  ...
  Hal_Pin_Info *PIN_MAP = HAL_Pin_Map(); // For DeviceOS 1.4.2
  // NRF5x_Pin_Info *PIN_MAP = HAL_Pin_Map(); // For DeviceOS 1.2.1
...
int PDMClass::begin(int channels, long sampleRate)
{
  ...
  nrf_pdm_psel_connect(get_nrf_pin_num(_clkPin), get_nrf_pin_num(_dinPin));
